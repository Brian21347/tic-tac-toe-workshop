...
def weak_minimax_find_move(position: Position):
    ...
    for move in position.available_moves():
        val = weak_minimax_eval(position.try_move(move))
...
def weak_minimax_eval(position: Position):
    do_max = position.move_index % 2 == 0
    ...
    value = -1 if do_max else 1
    for move in position.available_moves():
        score = weak_minimax_eval(position.try_move(move))
...
def main():
    ...
    while position.check_game_ends() is None:
        move = weak_minimax_find_move(position)
